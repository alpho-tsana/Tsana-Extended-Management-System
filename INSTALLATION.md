...
Setup instructions for TEMS - Tsana Extended Management System v0.3.
...
The CLI (`tems.py`) uses only Python standard library modules - no pip packages required.

For the TUI (`tems_tui.py`), install the Textual framework:
```bash
pip install textual
```
...
```
  ...
  ╚══██╔══╝ ██╔════╝ ████╗ ████║ ██╔════╝
     ...
     ██║    ███████╗ ██║ ╚═╝ ██║ ███████║
     ╚═╝    ╚══════╝ ╚═╝     ╚═╝ ╚══════╝
  Tsana Extended Management System v0.3
...
You should see the same menu in a rich terminal interface with clickable items and keyboard shortcuts. Press **q** to quit.

Note: The menu loops back after each action in both CLI and TUI modes.
...
- [ ] *(Optional)* `pip install textual` completed and `python3 tems_tui.py` launches the TUI
